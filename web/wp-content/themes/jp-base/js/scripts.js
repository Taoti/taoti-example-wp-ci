...
if( !window.jQuery && (jp_js!==undefined)  ){
    document.write('<script src="'+jp_js.path+'/jquery-3.3.1.min.js"><\/script>');
}
...
// Warning - this function is weird. Don't include a parentSelector parameter if you want it to work. If you include the parentSelector parameterit won't work, because I think the author meant "parentNode" or "parentElement". From what I can tell, the while loop happens one extra time when it shouldn't. When it finds the parent you were looking for, the while loop continues despite the fact that it's conditional no longer passes. It continues looping as long as p isn't the parentSelector, but even when p IS the parentSelector it continues and throws an error that it couldn't get the .parentNode of undefined.
